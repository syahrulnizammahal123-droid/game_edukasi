        ...
        <main class="flex-1 p-5 lg:p-8 overflow-y-auto">
            ...
            <div class="glass card-hover rounded-[35px] p-6 lg:p-8 mb-8">
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                    <div class="flex-1">
                        <p class="text-cyan-300 font-semibold mb-3">{{ $greeting }}</p>
                    ...
                    <div class="glass rounded-[30px] p-5 min-w-[280px] w-full lg:w-auto">
                        <div class="flex items-center gap-4">
                            <div class="w-16 h-16 rounded-3xl bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center shrink-0">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
                <div class="glass card-hover rounded-[30px] p-5 flex flex-col justify-between">
                    <span class="text-white/60 text-sm">Level Player</span>
                    <h2 class="text-white text-4xl font-extrabold mt-4">{{ $level }}</h2>
                </div>
                <div class="glass card-hover rounded-[30px] p-5 flex flex-col justify-between">
                    <span class="text-white/60 text-sm">Total Exp</span>
                    <h2 class="text-cyan-400 text-4xl font-extrabold mt-4">{{ $xp }}</h2>
                </div>
                <div class="glass card-hover rounded-[30px] p-5 flex flex-col justify-between">
                    <span class="text-white/60 text-sm">High Score</span>
                    <h2 class="text-yellow-400 text-4xl font-extrabold mt-4">{{ $progress->high_score }}</h2>
                </div>
                <div class="glass card-hover rounded-[30px] p-5 flex flex-col justify-between">
                    <span class="text-white/60 text-sm">Login Streak</span>
                    <h2 class="text-orange-400 text-4xl font-extrabold mt-4">🔥 {{ $progress->login_streak }} H</h2>
                </div>
            </div>
            ...
            <div class="glass card-hover rounded-[35px] p-6 lg:p-8 mb-8">
                <div class="space-y-3">
                    <div class="flex justify-between text-sm">
                        <span class="text-white/60">Progres Level Berikutnya</span>
                        <span class="text-cyan-300 font-bold">{{ $progressPercent }}%</span>
                    </div>
                    <div class="w-full h-3 rounded-full bg-white/10 overflow-hidden">
                        <div class="h-full rounded-full bg-gradient-to-r from-cyan-400 to-blue-600" style="width: {{ $progressPercent }}%"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="glass card-hover rounded-[35px] p-6 lg:p-8 mb-8">
                <div class="flex items-center justify-between mb-8">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-cyan-300 text-xs font-semibold">Season Progression</p>
                            <h2 class="text-white text-2xl font-bold">Adventure Pass</h2>
                        </div>
                    </div>
                    <div class="px-5 py-2 rounded-2xl bg-gradient-to-r from-cyan-500/20 to-blue-600/20 border border-cyan-400/20 text-white font-extrabold">
                        Tier {{ $level }}
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                    <div class="rounded-2xl p-4 text-center {{ $level >= 1 ? 'bg-gradient-to-br from-cyan-500/20 to-blue-600/20 border border-cyan-400/20' : 'bg-white/5 border border-white/10 opacity-40' }}">
                        ...
                        <p class="text-white/50 text-xs mt-1">+50 XP</p>
                    </div>
                    ...
                    <div class="rounded-2xl p-4 text-center {{ $level >= 3 ? 'bg-gradient-to-br from-emerald-500/20 to-cyan-600/20 border border-emerald-400/20' : 'bg-white/5 border border-white/10 opacity-40' }}">
                        ...
                        <p class="text-white/50 text-xs mt-1">Crystal</p>
                    </div>
                    ...
                    <div class="rounded-2xl p-4 text-center {{ $level >= 5 ? 'bg-gradient-to-br from-yellow-400/20 to-orange-500/20 border border-yellow-400/20' : 'bg-white/5 border border-white/10 opacity-40' }}">
                        ...
                        <p class="text-white/50 text-xs mt-1">Gold Reward</p>
                    </div>
                    ...
                    <div class="rounded-2xl p-4 text-center {{ $level >= 10 ? 'bg-gradient-to-br from-pink-500/20 to-purple-700/20 border border-pink-400/20' : 'bg-white/5 border border-white/10 opacity-40' }}">
                        ...
                        <p class="text-white/50 text-xs mt-1">Mythic</p>
                    </div>
                </div>
            </div>
                        ...
                        @foreach($topPlayers as $index => $player)
                            <div class="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/10 gap-4">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 class="text-white font-bold text-base">{{ $player->user->name }}</h3>
                                        <p class="text-white/50 text-xs">Level {{ floor(($player->high_score * 10) / 100) + 1 }}</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <h3 class="text-cyan-300 text-xl font-extrabold">{{ $player->high_score }}</h3>
                                    <p class="text-white/40 text-[10px]">Score</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="space-y-4 max-h-[350px] overflow-y-auto pr-2">
                        @if($xp > 0)
                            <div class="flex items-start gap-4 p-4 rounded-2xl bg-white/5 border border-white/10">
                                ...
                                <div class="flex-1">
                                    <h2 class="text-white font-bold text-base">Quiz Selesai</h2>
                                    <p class="text-white/60 text-xs mt-1">Kamu berhasil menyelesaikan sesi permainan terbaru.</p>
                                </div>
                                <span class="text-emerald-400 font-bold text-xs">+{{ $xp }} EXP</span>
                            </div>
                        @else
